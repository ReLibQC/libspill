...
#include "psio_libspill.h"     /* for psio_set_scratch_dir, the shim's own */
...
/* Symbols the kept libpsio files supply in a real Psi4 build. This translation
 * unit links the shim alone, so they are stubbed here rather than dragged in.
 * If the shim ever starts needing more of them, that is a signal the port is
 * taking over something it said it would leave alone. */
namespace psi {
void PSIO::filecfg_kwd(const char *, const char *, int, const char *) {}   // filescfg.cc
std::string PSIO::getpid() { return "0"; }                                  // getpid.cc
}  // namespace psi
extern "C" { char *psi_file_prefix = const_cast<char *>("psi"); }           // psi4 global
